...
def _count_overlaps_binary_search(tag_starts, tag_ends, anchor_start, anchor_end):
    """
    Fast overlap counting using binary search on sorted arrays.
    
    Overlap condition: tag_start <= anchor_end AND tag_end >= anchor_start
    
    This is much faster than full array scan for large tag arrays.
    """
    # Find tags that could possibly overlap using binary search
    # Tags that start after anchor_end cannot overlap
    right_idx = np.searchsorted(tag_starts, anchor_end, side='right')
    
    if right_idx == 0:
        return 0
    
    # Among tags starting before/at anchor_end, count those ending at/after anchor_start
    candidate_ends = tag_ends[:right_idx]
    count = int(np.sum(candidate_ends >= anchor_start))
...
def _process_cluster_chunk(args):
    ...
    Uses binary search for fast overlap counting.
    """
    ...
    for i in range(n_clusters):
        # Count overlaps for anchor 1
        chr1 = chunk_chr1[i]
        if chr1 in tags_by_chr:
            count1 = _count_overlaps_binary_search(
                tags_by_chr[chr1]['starts'],
                tags_by_chr[chr1]['ends'],
                chunk_start1[i],
                chunk_end1[i]
            )
        else:
        ...
        # Count overlaps for anchor 2
        chr2 = chunk_chr2[i]
        if chr2 in tags_by_chr:
            count2 = _count_overlaps_binary_search(
                tags_by_chr[chr2]['starts'],
                tags_by_chr[chr2]['ends'],
                chunk_start2[i],
                chunk_end2[i]
            )
        else:
...
class StatisticalSignificance:
    ...
    2. Hypergeometric test for enrichment
    3. Benjamini-Hochberg FDR correction
    4. P-value filtering
    
    Reference: Pvalues.java and hypergeometric.r
    """
    ...
    def calculate_significance(self,
        ...
        # Step 4: Run hypergeometric test
        logger.info(f"\nStep 3: Running hypergeometric test...")
